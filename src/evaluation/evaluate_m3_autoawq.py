...
def main():
    ...
    model = AutoAWQForCausalLM.from_quantized(MODEL_ID, fuse_layers=True, trust_remote_code=True, safetensors=True)
    ...
    res = {"avg_latency": np.mean(latencies), "rouge_l": rouge_l, "bert_score_f1": F1.mean().item() * 100}
    wandb.log(res)
