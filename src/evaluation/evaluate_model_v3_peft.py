        ...
        for name, mod in sys.modules.items():
            if 'modeling_exaone' in name:
                ...
                if not hasattr(mod, 'ALL_ATTENTION_FUNCTIONS'):
                    ...
                    mod.ALL_ATTENTION_FUNCTIONS.get_interface = lambda *a, **k: mod.eager_attention_forward
                ...
                if hasattr(mod, 'ExaoneModel'):
                    if not hasattr(mod.ExaoneModel, 'get_input_embeddings'):
                        ...
                        mod.ExaoneModel.set_input_embeddings = lambda self, value: setattr(self, 'wte', value)
                ...
                if hasattr(mod, 'ExaoneForCausalLM'):
                    if not hasattr(mod.ExaoneForCausalLM, 'get_input_embeddings'):
                        ...
                        mod.ExaoneForCausalLM.set_input_embeddings = lambda self, value: setattr(self.transformer, 'wte', value)
...
def main():
    wandb.init(project="exaone-civil-complaint", name=f"m3-peft-final-direct-{datetime.now().strftime('%Y%m%d-%H%M')}")
    ...
    for i, item in enumerate(test_data):
        ...
        class_prompt = tokenizer.apply_chat_template([
            {"role": "system", "content": "민원 분류기입니다. 반드시 한 단어(환경, 교통, 시설, 민원, 복지, 기타)로만 답하세요."},
