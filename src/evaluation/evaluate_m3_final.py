...
def parse_m3_category(response):
    ko_to_en = {"환경": "environment", "교통": "traffic", "도로": "traffic", "시설": "facilities", "민원": "civil_service", "복지": "welfare", "기타": "other"}
...
def main():
    ...
    wandb.init(
        ...
        config={"phase": "M3 Roadmap Final", "model": "EXAONE-Deep-7.8B", "repetition_penalty": 1.1}
    )
    ...
    for name, mod in sys.modules.items():
        if 'modeling_exaone' in name:
            if not hasattr(mod, 'ALL_ATTENTION_FUNCTIONS'):
                class Mock:
                    def get_interface(self, *a, **k): return mod.eager_attention_forward
            ...
            mod.ExaoneForCausalLM.set_input_embeddings = lambda self, v: setattr(self.transformer, 'wte', v)
            
            # prepare_inputs
            def pi(self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, **kwargs):
    ...
    for i, item in enumerate(test_data):
        messages = [{"role": "system", "content": "당신은 민원 공무원입니다."}, {"role": "user", "content": f"{item['instruction']}\n\n{item['input'][:1000]}"}]
        ...
        clean_refs.append(re.sub(r'<thought>.*?</thought>', '', item['output'], flags=re.DOTALL).strip())
        
        if parse_m3_category(gen_text) == extract_true_category(item['input']): correct += 1
    ...
    res = {"avg_latency": np.mean(latencies), "rouge_l": rouge_l, "bert_score_f1": F1.mean().item() * 100, "classification_accuracy": (correct/len(test_data))*100}
    wandb.log(res)
